...
grob <- grobTree(textGrob("Accident", x=0.1,  y=0.1, hjust=0,
                          gp=gpar(col="#FF5533", fontsize=10)))
grob2 <- grobTree(textGrob("Natural", x=0.7,  y=0.9, hjust=0,
                           gp=gpar(col="#FFBB33", fontsize=10)))
...
beta_dispersion_calc <- function(physeq) {
  ...
  beta <- betadisper(GPdist, sampledf$correct)
  print(return(permutest(beta)))
}
beta_dispersion_calc(physeq_NA_nos)
...
grob3 <- grobTree(textGrob("Cardiovascular Disease", x=0.1,  y=0.1, hjust=0,
                           gp=gpar(col="#5DD06D", fontsize=10)))
grob4 <- grobTree(textGrob("Drug-related", x=0.6,  y=0.9, hjust=0,
                           gp=gpar(col="#5D8BD0", fontsize=10)))
...
beta_dispersion_calc(physeq_CD_nos)
...
grob5 <- grobTree(textGrob("Diseased", x=0.1,  y=0.1, hjust=0,
                           gp=gpar(col="#935DD0", fontsize=10)))
grob6 <- grobTree(textGrob("Not Diseased", x=0.7,  y=0.9, hjust=0,
                           gp=gpar(col="#D0A95D", fontsize=10)))
...
beta_dispersion_calc(physeq_nos)

theme_set(theme_classic(base_size = 10))
tiff("FIG3.TIF", width = 3000, height = 2000, res=300)
